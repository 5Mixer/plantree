...
Main.__name__ = ["Main"];
Main.main = function() {
	kha_System.init({ title : "Project", width : 800, height : 140},function() {
...
var Person = function(name) {
	...
	this.x = 30;
	this.name = name;
	this.tid = Math.floor(Math.random() * 7);
};
...
Person.prototype = {
	...
	,tid: null
	,render: function(g) {
		g.drawSubImage(kha_Assets.images.people,this.x,kha_System.windowHeight() / 4 - 8,this.tid * 8,0,8,8);
		if(this.movingLeft) {
			this.x -= 1;
		}
		if(this.movingRight) {
...
var Project = function() {
	...
	this.selfid = 0;
	this.trees = [];
	this.people = new haxe_ds_IntMap();
	var _gthis = this;
	kha_System.notifyOnRender($bind(this,this.render));
	kha_Scheduler.addTimeTask($bind(this,this.update),0,0.016666666666666666);
	this.clientSocket = new mphx_client_impl_WebsocketClient(this.ip,8000);
	this.clientSocket.events.on("join",function(data) {
		var this1 = _gthis.people;
		var value = new Person("planter.");
		this1.h[data] = value;
	});
	this.clientSocket.events.on("assignid",function(data1) {
		_gthis.self = new Person("Me.");
		_gthis.selfid = data1;
		_gthis.people.h[data1] = _gthis.self;
	});
	this.clientSocket.events.on("move",function(data2) {
		if(data2.id == _gthis.selfid) {
			return;
		}
		if(_gthis.people.h.hasOwnProperty(data2.id) == false) {
			var p = new Person("planter.");
			p.x = data2.x;
			_gthis.people.h[data2.id] = p;
		}
		_gthis.people.h[data2.id].x = data2.x;
	});
	this.clientSocket.events.on("exit",function(data3) {
		_gthis.people.remove(data3);
	});
	this.clientSocket.events.on("plant",function(data4) {
		_gthis.trees.push(new Tree(data4.x,data4.name));
	});
	this.clientSocket.connect();
	this.clientSocket.send("join");
	kha_input_Keyboard.get().notify($bind(this,this.kdown),$bind(this,this.kup));
};
...
Project.prototype = {
	...
	,render: function(framebuffer) {
		...
		g.begin(true,kha__$Color_Color_$Impl_$.fromBytes(92,192,249));
		g.pushTransformation(new kha_math_FastMatrix3(4,0,0,0,4,0,0,0,1));
		var _g = 0;
		var _g1 = this.trees;
		while(_g < _g1.length) {
			var tree = _g1[_g];
		...
		var person = this.people.iterator();
		while(person.hasNext()) {
			var person1 = person.next();
			person1.render(g);
		}
		g.end();
		g.popTransformation();
		...
		if(this.people.h[this.selfid].movingLeft || this.people.h[this.selfid].movingRight) {
			this.clientSocket.send("move",this.people.h[this.selfid].x);
		}
	}
	,plantTree: function(x,name) {
		this.clientSocket.send("plant",{ name : "tree", x : x});
	}
	,kdown: function(key,$char) {
		if(this.selfid == 0) {
			return;
		}
		var me = this.people.h[this.selfid];
		if($char == "a") {
			me.movingLeft = true;
		}
		if($char == "d") {
		...
		if($char == " ") {
			this.plantTree(Math.floor(me.x),me.name);
		}
	}
	,kup: function(key,$char) {
		if(this.selfid == 0) {
			return;
		}
		var me = this.people.h[this.selfid];
...
var Tree = function(x,name) {
	this.x = 30;
	this.x = x;
	this.name = name;
	this.tid = Math.floor(Math.random() * 16);
};
$hxClasses["Tree"] = Tree;
Tree.__name__ = ["Tree"];
Tree.prototype = {
	...
	,tid: null
	,render: function(g) {
		g.drawSubImage(kha_Assets.images.trees,this.x,kha_System.windowHeight() / 4 - 32,this.tid * 16,0,16,32);
	}
	,__class__: Tree
};
...
var kha_ImageList = function() {
	this.names = ["people","trees"];
	this.treesDescription = { files : ["trees.png"], original_height : 32, type : "image", original_width : 256, name : "trees"};
	this.treesName = "trees";
	this.trees = null;
	this.peopleDescription = { files : ["people.png"], original_height : 8, type : "image", original_width : 56, name : "people"};
...
kha_ImageList.prototype = {
	...
	,peopleLoad: function(done) {
		kha_Assets.loadImage("people",function(image) {
			done();
		});
	}
	,peopleUnload: function() {
		this.people.unload();
		this.people = null;
	}
	,trees: null
	,treesName: null
	,treesDescription: null
	,treesLoad: function(done) {
		kha_Assets.loadImage("trees",function(image) {
			done();
		});
	}
	,treesUnload: function() {
		this.trees.unload();
		this.trees = null;
	}
	,names: null
...
kha_LoaderImpl.loadBlobFromDescription = function(desc,done) {
	...
	var tmp = app.getAppPath();
	var tmp1 = path.join(tmp,desc.files[0]);
	fs.readFile(tmp1,function(err,data) {
...
kha_Shaders.init = function() {
	...
	var data2 = Reflect.field(kha_Shaders,"painter_text_fragData");
	var bytes2 = haxe_Unserializer.run(data2);
	kha_Shaders.painter_text_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes2),"painter_text_frag");
...
var mphx_client_impl_WebsocketClient = function(_ip,_port) {
	this.fastSend = true;
	this.ready = false;
	this.events = new mphx_utils_event_impl_ClientEventManager();
	this.serializer = new mphx_serialization_impl_HaxeSerializer();
	this.ip = _ip;
	this.port = _port;
	this.messageQueue = [];
};
$hxClasses["mphx.client.impl.WebsocketClient"] = mphx_client_impl_WebsocketClient;
mphx_client_impl_WebsocketClient.__name__ = ["mphx","client","impl","WebsocketClient"];
mphx_client_impl_WebsocketClient.__interfaces__ = [mphx_client_IClient];
mphx_client_impl_WebsocketClient.prototype = {
	websocket: null
	,events: null
	,serializer: null
	,port: null
	...
	,onConnectionEstablished: null
	,fastSend: null
	,set_fastSend: function(newValue) {
		mphx_utils_Log.message(1,"SetFastSend is not implemented for web sockets.",{ fileName : "WebsocketClient.hx", lineNumber : 27, className : "mphx.client.impl.WebsocketClient", methodName : "set_fastSend"});
		return false;
	}
	,connect: function() {
		var _gthis = this;
		mphx_utils_Log.message(4,"Attempting to connect on: " + this.ip + ":" + this.port,{ fileName : "WebsocketClient.hx", lineNumber : 43, className : "mphx.client.impl.WebsocketClient", methodName : "connect"});
		this.websocket = new WebSocket("ws://" + this.ip + ":" + this.port);
		this.websocket.onerror = function(e) {
			if(_gthis.onConnectionError != null) {
				_gthis.onConnectionError(e);
			}
			mphx_utils_Log.message(2,"Connection error: " + Std.string(e),{ fileName : "WebsocketClient.hx", lineNumber : 48, className : "mphx.client.impl.WebsocketClient", methodName : "connect"});
		};
		this.websocket.onopen = function() {
			if(_gthis.onConnectionEstablished != null) {
				_gthis.onConnectionEstablished();
			}
			_gthis.ready = true;
			mphx_utils_Log.message(4,"Connected on: " + _gthis.ip + ":" + _gthis.port,{ fileName : "WebsocketClient.hx", lineNumber : 55, className : "mphx.client.impl.WebsocketClient", methodName : "connect"});
			var _g = 0;
			var _g1 = _gthis.messageQueue;
			while(_g < _g1.length) {
				var message = _g1[_g];
				++_g;
				_gthis.send(message.t,message.data);
				HxOverrides.remove(_gthis.messageQueue,message);
			}
		};
		this.websocket.onmessage = function(line) {
			var data = line.data;
			var msg = _gthis.serializer.deserialize(data);
			_gthis.events.callEvent(msg.t,msg.data);
		};
	}
	,send: function(event,data) {
		if(this.isConnected() == false) {
			mphx_utils_Log.message(1 | 8,"Cannot sent event " + event + " as client is not connected to a server.",{ fileName : "WebsocketClient.hx", lineNumber : 78, className : "mphx.client.impl.WebsocketClient", methodName : "send"});
			return;
		}
	...
	,close: function() {
		...
		if(this.onConnectionClose != null) {
			this.onConnectionClose(mphx_utils_ClientError.DroppedConnection);
		}
	}
	,__class__: mphx_client_impl_WebsocketClient
};
...
var mphx_serialization_impl_HaxeSerializer = function() {
};
$hxClasses["mphx.serialization.impl.HaxeSerializer"] = mphx_serialization_impl_HaxeSerializer;
mphx_serialization_impl_HaxeSerializer.__name__ = ["mphx","serialization","impl","HaxeSerializer"];
mphx_serialization_impl_HaxeSerializer.__interfaces__ = [mphx_serialization_ISerializer];
mphx_serialization_impl_HaxeSerializer.prototype = {
	...
	,__class__: mphx_serialization_impl_HaxeSerializer
};
var mphx_utils_ClientError = $hxClasses["mphx.utils.ClientError"] = { __ename__ : ["mphx","utils","ClientError"], __constructs__ : ["ServerUnavaliable","DroppedConnection","Other"] };
mphx_utils_ClientError.ServerUnavaliable = ["ServerUnavaliable",0];
mphx_utils_ClientError.ServerUnavaliable.toString = $estr;
mphx_utils_ClientError.ServerUnavaliable.__enum__ = mphx_utils_ClientError;
mphx_utils_ClientError.DroppedConnection = ["DroppedConnection",1];
mphx_utils_ClientError.DroppedConnection.toString = $estr;
mphx_utils_ClientError.DroppedConnection.__enum__ = mphx_utils_ClientError;
mphx_utils_ClientError.Other = function(reason) { var $x = ["Other",2,reason]; $x.__enum__ = mphx_utils_ClientError; $x.toString = $estr; return $x; };
var mphx_utils_ServerError = $hxClasses["mphx.utils.ServerError"] = { __ename__ : ["mphx","utils","ServerError"], __constructs__ : ["CorruptData","Other"] };
mphx_utils_ServerError.CorruptData = ["CorruptData",0];
mphx_utils_ServerError.CorruptData.toString = $estr;
mphx_utils_ServerError.CorruptData.__enum__ = mphx_utils_ServerError;
mphx_utils_ServerError.Other = function(reason) { var $x = ["Other",1,reason]; $x.__enum__ = mphx_utils_ServerError; $x.toString = $estr; return $x; };
var mphx_utils__$Log_DebugLevel_$Impl_$ = {};
$hxClasses["mphx.utils._Log.DebugLevel_Impl_"] = mphx_utils__$Log_DebugLevel_$Impl_$;
mphx_utils__$Log_DebugLevel_$Impl_$.__name__ = ["mphx","utils","_Log","DebugLevel_Impl_"];
mphx_utils__$Log_DebugLevel_$Impl_$.value = function(index) {
	return 1 << index;
};
var mphx_utils_Log = function() { };
$hxClasses["mphx.utils.Log"] = mphx_utils_Log;
mphx_utils_Log.__name__ = ["mphx","utils","Log"];
mphx_utils_Log.message = function(level,message,infos) {
	var l = "";
	if(mphx_utils_Log.printLevel) {
		if((level & 1) != 0) {
			l += "[Warning]";
		}
		if((level & 2) != 0) {
	...
	var location = "";
	if(mphx_utils_Log.printLocation) {
		location = infos.fileName + ":" + infos.lineNumber + " : ";
	}
	if(mphx_utils_Log.usePrintLn) {
		mphx_utils_Log.usePrintLn = false;
		haxe_Log.trace("Can't use print line on non sys targets.",{ fileName : "Log.hx", lineNumber : 40, className : "mphx.utils.Log", methodName : "message"});
	} else if((mphx_utils_Log.debugLevel & level) != 0) {
		haxe_Log.trace(l + location + message,{ fileName : "Log.hx", lineNumber : 45, className : "mphx.utils.Log", methodName : "message"});
	}
};
var mphx_utils_event_impl_ClientEventManager = function() {
	this.eventMap = new haxe_ds_StringMap();
	this.callEvent = $bind(this,this.callEventCallback);
};
$hxClasses["mphx.utils.event.impl.ClientEventManager"] = mphx_utils_event_impl_ClientEventManager;
mphx_utils_event_impl_ClientEventManager.__name__ = ["mphx","utils","event","impl","ClientEventManager"];
mphx_utils_event_impl_ClientEventManager.prototype = {
	callEvent: null
	,eventMap: null
	,on: function(eventName,event) {
		var _this = this.eventMap;
		if(__map_reserved[eventName] != null) {
			_this.setReserved(eventName,event);
		} else {
			_this.h[eventName] = event;
		}
	}
	,remove: function(eventName) {
		var _this = this.eventMap;
		if(__map_reserved[eventName] != null ? _this.existsReserved(eventName) : _this.h.hasOwnProperty(eventName)) {
			var _this1 = this.eventMap;
			if(__map_reserved[eventName] != null) {
				_this1.setReserved(eventName,null);
			} else {
				_this1.h[eventName] = null;
			}
			this.eventMap.remove(eventName);
		}
	}
	,callEventCallback: function(eventName,data) {
		var _this = this.eventMap;
		if(__map_reserved[eventName] != null ? _this.existsReserved(eventName) : _this.h.hasOwnProperty(eventName)) {
			var _this1 = this.eventMap;
			if((__map_reserved[eventName] != null ? _this1.getReserved(eventName) : _this1.h[eventName]) != null) {
				var _this2 = this.eventMap;
				(__map_reserved[eventName] != null ? _this2.getReserved(eventName) : _this2.h[eventName])(data);
			} else {
				mphx_utils_Log.message(4 | 8,"mphx recieved event type " + eventName + " however no event listener was registered for it.",{ fileName : "ClientEventManager.hx", lineNumber : 58, className : "mphx.utils.event.impl.ClientEventManager", methodName : "callEventCallback"});
			}
		}
	}
	,__class__: mphx_utils_event_impl_ClientEventManager
};
...
mphx_utils__$Log_DebugLevel_$Impl_$.None = 0;
mphx_utils__$Log_DebugLevel_$Impl_$.Warnings = 1;
mphx_utils__$Log_DebugLevel_$Impl_$.Errors = 2;
mphx_utils__$Log_DebugLevel_$Impl_$.Info = 4;
mphx_utils__$Log_DebugLevel_$Impl_$.Networking = 8;
mphx_utils_Log.debugLevel = 2 | 1 | 4;
mphx_utils_Log.usePrintLn = false;
mphx_utils_Log.printLevel = true;
mphx_utils_Log.printLocation = true;
Main.main();
